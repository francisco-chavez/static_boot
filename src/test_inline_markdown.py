...
class Test_SplitLinksAndImages(unittest.TestCase):
    ...
    def test_image_03(self):
        node1 = TextNode("No images here", TextType.TEXT)
        node2 = TextNode("Still no images", TextType.TEXT)

        new_nodes = split_nodes_image([node1, node2])
        self.assertListEqual(
            [
                TextNode("No images here", TextType.TEXT),
                TextNode("Still no images", TextType.TEXT)
            ],
            new_nodes
        )

    def test_image_04(self):
        node1 = TextNode("No images here", TextType.TEXT)
        node2 = TextNode("![alt text](https://image.png)", TextType.TEXT)
...
class TestInlineMarkdown(unittest.TestCase):
    ...
    def test_text_to_textnodes(self):
        nodes = text_to_textnodes(
            "This is **text** with an _italic_ word and a `code block` and an ![image](https://i.imgur.com/zjjcJKZ.png) and a [link](https://boot.dev)**, and more.**"
        )
        self.assertListEqual(
            [
                ...
                TextNode("link", TextType.LINK, "https://boot.dev"),
                TextNode(", and more.", TextType.BOLD)
            ],
            nodes,
        )
